...
class EMBLEM
{
	private $tmp_dir = __DIR__.'/cache/tmp_emblem/';
	...
	private function prepare_emblem($guild_id)
	{		
		$DB = new DATABASE;
		$CONFIG = new CONFIG;
		
		if (empty($guild_id) || !is_numeric($guild_id))
			return FALSE;
		
		$file_name = $this->tmp_dir . $guild_id . ".png";
		$url_filename = $CONFIG->getConfig("Web")."libs/cache/tmp_emblem/". $guild_id . ".png";
		
		if (!is_dir($this->tmp_dir))
			mkdir($this->tmp_dir, 0700, true);
		
		if (file_exists($file_name) && (time() - filemtime($file_name)) < (12*60)) 
			return $url_filename;
		else
		{
			$consult = "SELECT `emblem_len`, `emblem_data` FROM `guild` WHERE `guild_id` = ? LIMIT 1";
			$result = $DB->execute($consult, [$guild_id]);
			$row = $result->fetch();
			if (!$result || empty($row['emblem_len']))
			{
				$DB->free($result);
				return 'X';
			}
			...
			{
				$data  = @gzuncompress(pack('H*', $row['emblem_data']));
				$image = $this->imagecreatefrombmpstring($data);
				
				//Guardamos la imagen en cache
	...
	public function get_emblem($guild_id)
	{
		return $this->prepare_emblem($guild_id);
	}	
}
